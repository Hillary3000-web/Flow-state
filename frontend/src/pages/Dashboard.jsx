...
const StatCard = ({ icon: Icon, label, value, color, delay, isMobile, to }) => (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}>
        <Link to={to} style={{ textDecoration: 'none', display: 'block' }}>
            <div style={{
                ...cardStyle, padding: isMobile ? '16px' : '24px',
...
export default function Dashboard() {
    ...
    const badge = (bg, color) => ({
        ...
        borderRadius: '50px', fontSize: '0.6875rem', fontWeight: 600, background: bg, color,
    });

    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? '20px' : '32px' }}>
            <SEO title="Dashboard" description="Your productivity overview." />

            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <h1 style={{ fontSize: isMobile ? '1.25rem' : '1.5rem', fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
                    {greeting()} 👋
                </h1>
            ...
            {isLoading ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: isMobile ? '10px' : '16px' }}>
                    <StatCard icon={HiOutlineCheckCircle} label="Completed" value={stats?.completed || 0} color="#22c55e" delay={0.05} isMobile={isMobile} to="/tasks" />
                    <StatCard icon={HiOutlineClock} label="In Progress" value={stats?.in_progress || 0} color="#eab308" delay={0.1} isMobile={isMobile} to="/tasks" />
                    <StatCard icon={HiOutlineExclamation} label="Overdue" value={stats?.overdue || 0} color="#ef4444" delay={0.15} isMobile={isMobile} to="/tasks" />
                    <StatCard icon={HiOutlineLightningBolt} label="Focus Today" value={`${stats?.today_focus_minutes || 0}m`} color="#6366f1" delay={0.2} isMobile={isMobile} to="/focus" />
                </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? '2fr 1fr' : '1fr', gap: isMobile ? '16px' : '24px' }}>
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                        ...
                        <span style={{ display: 'inline-flex', alignItems: 'center', padding: '3px 10px', borderRadius: '50px', fontSize: '0.6875rem', fontWeight: 600, background: 'var(--accent-glow)', color: 'var(--accent-light)' }}>14 days</span>
                    </div>
                        ...
                        {trends.length > 0 ? (
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={trends}>
                                    ...
                                    <Area type="monotone" dataKey="completed" stroke="#6366f1" fill="url(#areaGrad)" strokeWidth={2} />
                                </AreaChart>
                            </ResponsiveContainer>
                        ) : (
                            ...
                            <>
                                {risks.overdue?.slice(0, 3).map(t => (
                                    <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', borderRadius: '10px', background: 'var(--danger-bg)' }}>
                                        ...
                                        <span style={badge('var(--danger-bg)', 'var(--danger)')}>Overdue</span>
                                    </div>
                                ))}
                                {risks.at_risk?.slice(0, 3).map(t => (
                                    <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', borderRadius: '10px', background: 'var(--warning-bg)' }}>
                                        ...
                                        <span style={{ fontSize: '0.875rem', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: 'var(--text-primary)' }}>{t.title}</span>
                                        <span style={badge('var(--warning-bg)', 'var(--warning)')}>At risk</span>
                                    </div>
                                ))}
                            </>
                        )}
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
                    <h2 style={{ fontSize: '0.9375rem', fontWeight: 600, color: 'var(--text-primary)' }}>Completion Rate</h2>
                ...
                <div style={{ height: '10px', borderRadius: '50px', overflow: 'hidden', background: 'var(--bg-hover)' }}>
                    <motion.div
                        style={{ height: '100%', borderRadius: '50px', background: 'linear-gradient(90deg, #6366f1, #a5b4fc)' }}
                        initial={{ width: 0 }}
